...
const totalIndividualContributions = GAMES_JSON.reduce(
...
function filterUnfundedOnly() {
  ...
  const filterUnfundedGames = GAMES_JSON.filter(
    (game) => game.pledged < game.goal
  );

  // use the function we previously created to add the unfunded games to the DOM
  addGamesToPage(filterUnfundedGames);
}
...
function filterFundedOnly() {
  ...
  const filterFundedGames = GAMES_JSON.filter(
    (game) => game.pledged >= game.goal
  );
...
topFundedGameRunner.innerHTML = secondGame.name;
secondGameContainer.appendChild(topFundedGameRunner);